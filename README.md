...
| 功能        | 命令格式                                     | 参数说明                                | 备注            | 别名     |
|-----------|------------------------------------------|-------------------------------------|---------------|--------|
| **账号绑定**  | `/bind [platform] [playername]`                    | `platform`: 5e/pw(可选，默认5e) <br>`playername`: 平台账号名                    | *该命令仅用于将游戏ID绑定至发送者QQ号*             | `/绑定`, `/添加`, `/绑定用户`, `/添加用户`  |
| **查询战绩**  | `/match [platform] [@qq名] [match_round]`        | `platform`: 5e/pw/mm(可选，默认5e) <br>`@qq名`: 艾特某个qq群聊用户 <br>`match_round`: 倒数第几把游戏    | -match_round 默认为1，即倒数第一把，仅支持查询最近5把<br>-若艾特的是bot，则查询命令发送者自己的战绩<br>-mm 查询复用 pw 绑定信息，请先 `/bind pw`             | `/战绩`, `/查询战绩`      |
| **帮助**    | `/cs_help`                           | -                                   | -             | -      |
